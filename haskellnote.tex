...
圏$\mathcal{C}$の対象を$X$とする．圏$\mathcal{D}$の対象は関手$\mathfrak{F}$によって対象$X$と関係づけられる．圏$\mathcal{C}$における射$f:X\rightarrow Y$が$\mathfrak{F}(f):\mathfrak{F}(X)\rightarrow\mathfrak{F}(Y)$に対応し，次の関係を満たす．
\begin{itemize}
\item $X\in\mathcal{C}$に対して$\mathfrak{F}(\mathId_X)=\mathId_{\mathfrak{F}(X)}$
\item $f:X\rightarrow Y$および$g:Y\rightarrow Z$に対して$\mathfrak{F}(g\mathCompose f)=\mathfrak{F}(g)\mathCompose\mathfrak{F}(f)$
\end{itemize}
