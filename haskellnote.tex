...
同様に，浮動小数点型のリテラルは次のように書く．\footnote{\haskell では \verb|x = 1.0| と書く．}
\begin{equation}
x=1.0
\end{equation}

文字型のリテラルは次のように書く．\footnote{\haskell では \verb|x = 'a'| と書く．}
\begin{equation}
x=\mathLiteralChar{a}
\end{equation}

論理型のリテラルは次のように書く．\footnote{\haskell では \verb|x = True| と書く．}
\begin{equation}
x=\mathLiteral{True}
\end{equation}
論理型リテラルは$\mathLiteral{True}$の他に$\mathLiteral{False}$がある．なお$\mathLiteral{True}$と$\mathLiteral{False}$は正しくは\keyword{値コンストラクタ}である．
