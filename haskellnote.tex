...
$f\mathList{x}=\textrm{sqrt}\mathCompose\textrm{fromIntegral}\mathCompose\textrm{sum}\mathApply(\mathLambda{x}{x*x})\mathMap\mathList{x}$
...
\begin{align}
  f&\mathTypeIs\mathTypeFunction{\mathTypeInt}{\mathTypeInt}\\
  f&=\dots\\
  \textrm{ri}&\mathTypeIs\mathTypeFunction{\mathTypeString}{\mathTypeInt}\\
  \textrm{ri}&=\textrm{read}\\
  \textsl{main}&=\textsl{print}\mathBind(((f\mathCompose\textrm{ri}\mathMap)\mathMap)\mathCompose(\textrm{words}\mathMap)\mathCompose\textrm{lines}\mathFMap\textsl{getContents})
\end{align}
...
main = print . ((f <$>) <$>) . ((readInt <$>) <$>) . (words <$>) . lines =<< getContents
\end{verbatim}
